...
export default function SettingsView({ onEventNotification }) {
  ...
  const fetchDeepseekStatus = async () => {
    try {
      const res = await fetch('/api/ai/deepseek-status');
  ...
  const handleTestDeepseek = async (e) => {
    e?.preventDefault();
    setIsTestingDeepseek(true);
    setDeepseekMessage(null);

    try {
      const res = await fetch('/api/ai/deepseek-configure', {
        method: 'POST',
      ...
      if (data.test?.success) {
        setDeepseekMessage({ type: 'success', text: 'DeepSeek API authenticated and operational.' });
        if (onEventNotification) onEventNotification('DeepSeek API connection verified.');
      } else {
        const errorText = data.test?.error || data.error || 'Connection failed';
        const isBalance = errorText.includes('Balance');
        setDeepseekMessage({
          type: isBalance ? 'warning' : 'error',
          text: isBalance 
            ? 'Key valid! DeepSeek balance is currently zero. Top up at platform.deepseek.com. Local heuristic AI is handling requests.' 
            : errorText,
        });
      }
      await fetchDeepseekStatus();
    } catch (err) {
      setDeepseekMessage({ type: 'error', text: err.message });
    } finally {
      ...
      <div className="p-6 bg-white border border-slate-200 rounded-2xl shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <h3 className="text-sm font-bold text-slate-900">DeepSeek AI Engine</h3>
                <span className="text-[10px] font-semibold bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full">
                  deepseek-chat
                </span>
              </div>
              <p className="text-xs text-slate-500">
                Powers Ingest normalizer, Outreach buyer intent extractor, and Generative visibility scoring.
              </p>
            </div>
          </div>
        ...
        {deepseekMessage && (
          <div className={`p-3 rounded-xl border text-xs flex items-center space-x-2 ${
            deepseekMessage.type === 'success' 
              ? 'bg-emerald-50 border-emerald-200 text-emerald-800' 
              : deepseekMessage.type === 'warning'
            ...
            {deepseekMessage.type === 'success' ? (
              ...
              <AlertTriangle className={`w-4 h-4 shrink-0 ${deepseekMessage.type === 'warning' ? 'text-amber-600' : 'text-rose-600'}`} />
            )}
            <span>{deepseekMessage.text}</span>
          </div>
        )}
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <button
                ...
                disabled={isTestingDeepseek || !deepseekApiKey}
                className="flex items-center justify-center space-x-1.5 px-4 py-2 text-xs font-semibold bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-xs transition-colors cursor-pointer shrink-0 disabled:opacity-50"
              >
                {isTestingDeepseek ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Zap className="w-3.5 h-3.5" />}
                <span>{isTestingDeepseek ? 'Testing...' : 'Test DeepSeek API'}</span>
              </button>
            </div>
            <span className="text-[11px] text-slate-400 mt-1 block">
              Endpoint: <code className="font-mono-code text-slate-600">https://api.deepseek.com/v1</code>. Autonomous heuristic fallback protects transactions if API balance is low.
            </span>
          </div>
        </div>
      </div>
